...
static void HandleErrors(OMPI_Comm *pcomm, int *perr, ... )
{
  ...
  {
    if(MPI_COMM_WORLD->OMPI_COMM_CMP != OMPI_COMM_NULL)
    {
      ...
      printf("CMP Starting EMPI comm creation %d\n",MPI_COMM_WORLD->OMPI_COMM_CMP == OMPI_COMM_NULL);
    ...
    else if(MPI_COMM_WORLD->OMPI_COMM_REP != OMPI_COMM_NULL)
    {
      ...
      printf("before shrink REP\n");
      OMPIX_Comm_shrink(MPI_COMM_WORLD->oworldComm,&new_intercomm);
      printf("after shrink REP\n");
      OMPI_Comm_group(MPI_COMM_WORLD->oworldComm, &worldGroup);
      OMPI_Comm_group(new_intercomm, &group_c);
      OMPI_Group_difference(worldGroup,group_c,&group_f);
      ...
      int myreprank;
      OMPI_Comm_rank(MPI_COMM_WORLD->OMPI_COMM_REP,&myreprank);
      worldComm_ranks = (int*)malloc((nR+nC) * sizeof(int));
      int rankadj[nC+nR];
      int rankadjrep[nC+nR];
		...
		else
		{
			...
			if(MPI_COMM_WORLD->EMPI_COMM_REP != EMPI_COMM_NULL)
			{
				if(!pdata->completerep || !pdata->completecmp)
				{
					...
					{
						retValRep = 0;
						if(cmpToRepMap[pdata->target] == -1) retValRep = check_errhandler_conditions((pdata->comm->OMPI_CMP_REP_INTERCOMM),((*(pdata->req))->reqcmp),pdata->target);
						else retValRep = check_errhandler_conditions((pdata->comm->OMPI_COMM_REP),((*(pdata->req))->reqrep),cmpToRepMap[pdata->target]);
						if(retValRep == 0)
						{
			...
			else if(MPI_COMM_WORLD->EMPI_COMM_CMP != EMPI_COMM_NULL)
			{
				if(!pdata->completerep || !pdata->completecmp)
				{
					...
					{
						retVal = 0;
						retVal = check_errhandler_conditions((pdata->comm->OMPI_COMM_CMP), ((*(pdata->req))->reqcmp),pdata->target);
						if(retVal == 0)
						{
  ...
  //Identify items from sendnums that were not in recvfrommenums and resend them
	...
	for(int i = 0; i<nC+nR;i++)
	{
		if(MPI_COMM_WORLD->EMPI_COMM_CMP != EMPI_COMM_NULL)
		{
			...
			{
				if(recvnums[i] != senttomenums[i] && i < nC)
				{
				...
				if(sendnums[i] != recvfrommenums[i])
				{
					EMPI_Status status;
					sendids = (int *)malloc(recvfrommenums[i]*sizeof(int));
					EMPI_Recv(sendids,recvfrommenums[i],EMPI_INT,i,i+myrank,MPI_COMM_WORLD->eworldComm,&status);
							...
							if(resend)
							{
								...
								*((int *)(pdata->buf+dis)) = pdata->id;
								do
								{
									...
									if(i < nC) retVal = check_errhandler_conditions((pdata->comm->OMPI_COMM_CMP),((*(pdata->req))->reqcmp),pdata->target);
									else if(cmpToRepMap[rank] == -1) retVal = check_errhandler_conditions((pdata->comm->OMPI_CMP_REP_INTERCOMM),((*(pdata->req))->reqrep),cmpToRepMap[pdata->target]);
									else break;
									if(retVal == 0)
									{
		...
		else if(MPI_COMM_WORLD->EMPI_COMM_REP != EMPI_COMM_NULL)
		{
			if(i != repToCmpMap[reprank] &&  i != reprank + nC)
			{
				if(recvnums[i] != senttomenums[i])
				{
					...
					while(pdata != NULL)
					{
						if(!pdata->sendtype && pdata->target == i)
						{
							recvids[index] = pdata->id;
							index++;
						}
						...
						if(pdata->sendtype && (cmpToRepMap[pdata->target] + nC) == i)
						{
							...
							if(resend)
							{
								...
								*((int *)(pdata->buf+dis)) = pdata->id;
								...
								{
									retVal = 0;
									retVal = check_errhandler_conditions((pdata->comm->OMPI_COMM_REP),((*(pdata->req))->reqrep),cmpToRepMap[pdata->target]);
									if(retVal == 0)
									{
		...
		if(data->type == MPI_FT_BCAST)
		{
			...
			if(MPI_COMM_WORLD->EMPI_COMM_REP != EMPI_COMM_NULL)
			{
				do
				{
					retVal = 0;
					retValinter = 0;
					if(data->id > repid && repid >= 0 && cmpToRepMap[(data->args).bcast.root] == -1) retVal = check_errhandler_conditions_collective(((data->args).bcast.comm)->OMPI_CMP_REP_INTERCOMM,&reqinter);
					else if(data->id > repid && repid >= 0) retVal = check_errhandler_conditions_collective(((data->args).bcast.comm)->OMPI_COMM_REP,&req);
			...
			{	
				do
				{
					...
					if(data->id > cmpid) retVal = check_errhandler_conditions_collective(((data->args).bcast.comm)->OMPI_COMM_CMP,&req);
					if(data->id > repid && repid >= 0 && cmpToRepMap[(data->args).bcast.root] == -1 && nR > 0) retValinter = check_errhandler_conditions_collective(((data->args).bcast.comm)->OMPI_CMP_REP_INTERCOMM,&reqinter);
					...
					if(retVal == 0 && retValinter == 0)
					{
						...
						do
						{
							if(data->id > cmpid && flag == 0) retVal = check_errhandler_conditions_collective(((data->args).bcast.comm)->OMPI_COMM_CMP,&req);
							if(retVal != 0) break;
							if(data->id > repid && repid >= 0 && cmpToRepMap[(data->args).bcast.root] == -1 && nR > 0 && flaginter == 0 && data->completerep) retValinter = check_errhandler_conditions_collective(((data->args).bcast.comm)->OMPI_CMP_REP_INTERCOMM,&req);
		...
		else if(data->type == MPI_FT_SCATTER)
		{
			...
			if(MPI_COMM_WORLD->EMPI_COMM_REP != EMPI_COMM_NULL)
			{
				do
				{
					...
					if(data->id > repid && repid >= 0 && cmpToRepMap[(data->args).scatter.root] == -1) retVal = check_errhandler_conditions_collective(((data->args).scatter.comm)->OMPI_CMP_REP_INTERCOMM,&reqinter);
					else if(data->id > repid && repid >= 0) retVal = check_errhandler_conditions_collective(((data->args).scatter.comm)->OMPI_COMM_REP,&req);
			...
			{	
				do
				{
					...
					EMPI_Comm_rank((data->args).scatter.comm->EMPI_COMM_CMP,&rank);
					if(data->id > cmpid) retVal = check_errhandler_conditions_collective(((data->args).scatter.comm)->OMPI_COMM_CMP,&req);
					if(data->id > repid && repid >= 0 && cmpToRepMap[(data->args).scatter.root] == -1 && nR > 0) retValinter = check_errhandler_conditions_collective(((data->args).scatter.comm)->OMPI_CMP_REP_INTERCOMM,&reqinter);
					...
					if(retVal == 0 && retValinter == 0)
					{
						...
						do
						{
							if(data->id > cmpid && flag == 0) retVal = check_errhandler_conditions_collective(((data->args).scatter.comm)->OMPI_COMM_CMP,&req);
							if(retVal != 0) break;
							if(data->id > repid && repid >= 0 && cmpToRepMap[(data->args).scatter.root] == -1 && nR > 0 && flaginter == 0 && data->completerep) retValinter = check_errhandler_conditions_collective(((data->args).scatter.comm)->OMPI_CMP_REP_INTERCOMM,&reqinter);
							if(retValinter != 0) break;
							if(data->id > cmpid  && flag == 0) EMPI_Test(&req,&flag,&status);
		...
		else if(data->type == MPI_FT_GATHER)
		{
			...
			if(MPI_COMM_WORLD->EMPI_COMM_REP != EMPI_COMM_NULL)
			{
				do
				{
					...
					if(cmpToRepMap[(data->args).gather.root] != -1 && data->id > repid && repid >= 0)
					{
						...
						if(commnC > commnR && commnR > 0) retValinter = check_errhandler_conditions_collective(((data->args).gather.comm)->OMPI_CMP_NO_REP_INTERCOMM,&reqinter);
						retVal = check_errhandler_conditions_collective(((data->args).gather.comm)->OMPI_COMM_REP,&req);
						...
						if(retVal == 0 && retValinter == 0)
						{
							...
							do
							{
								if(flag == 0) retVal = check_errhandler_conditions_collective(((data->args).gather.comm)->OMPI_COMM_REP,&req);
								if(retVal != 0) break;
								if(commnC > commnR && commnR > 0 && flaginter == 0) retValinter = check_errhandler_conditions_collective(((data->args).gather.comm)->OMPI_CMP_NO_REP_INTERCOMM,&reqinter);
								if(retValinter != 0) break;
			...
			{	
				do
				{
					...
					if(data->id > cmpid) retVal = check_errhandler_conditions_collective(((data->args).gather.comm)->OMPI_COMM_CMP,&req);
					if(data->id > repid && repid >= 0 && cmpToRepMap[(data->args).gather.root] != -1 && cmpToRepMap[rank] == -1) retValinter = check_errhandler_conditions_collective(((data->args).gather.comm)->OMPI_CMP_NO_REP_INTERCOMM,&reqinter);
					...
					if(retVal == 0 && retValinter == 0)
					{
						...
						do
						{
							if(data->id > cmpid && flag == 0) retVal = check_errhandler_conditions_collective(((data->args).gather.comm)->OMPI_COMM_CMP,&req);
							if(retVal != 0) break;
							if(data->id > repid && repid >= 0 && cmpToRepMap[(data->args).gather.root] != -1 && cmpToRepMap[rank] == -1 && data->completerep) retValinter = check_errhandler_conditions_collective(((data->args).gather.comm)->OMPI_CMP_NO_REP_INTERCOMM,&req);
							if(retValinter != 0) break;
							if(data->id > cmpid  && flag == 0) EMPI_Test(&req,&flag,&status);
		...
		else if(data->type == MPI_FT_REDUCE)
		{
			if(MPI_COMM_WORLD->EMPI_COMM_CMP != EMPI_COMM_NULL)
			{
				if(data->id > cmpid)
				{
					...
					{
						retVal = 0;
						retVal = check_errhandler_conditions_collective(((data->args).reduce.comm)->OMPI_COMM_CMP,&req);
				...
				if(data->id > myrepid && myrepid >= 0)
				{
					do
					{
						...
						retVal = check_errhandler_conditions(((data->args).reduce.comm)->OMPI_CMP_REP_INTERCOMM,&req,cmpToRepMap[rank]);
		...
		else if(data->type == MPI_FT_ALLGATHER)
		{
				...
				do
				{
					...
					if(data->id > repid && repid >= 0)
					{
						...
						if(commnC > commnR && commnR > 0)
						{
							...
							{
								completereq[i] = false;
								retValinter = check_errhandler_conditions_collective(((data->args).allgather.comm->OMPI_CMP_NO_REP_INTERCOMM),&reqs[i]);
								if(retValinter == 0)
								{
						...
						retVal = check_errhandler_conditions_collective(((data->args).allgather.comm)->OMPI_COMM_REP,&req);
						if(retVal == 0)
						{
			...
			{	
				do
				{
					...
					if(data->id > cmpid) retVal = check_errhandler_conditions_collective(((data->args).allgather.comm)->OMPI_COMM_CMP,&req);
					if(retVal == 0)
					{
						...
						do
						{
							if(data->id > cmpid && flag == 0) retVal = check_errhandler_conditions_collective(((data->args).allgather.comm)->OMPI_COMM_CMP,&req);
							if(retVal != 0) break;
							if(data->id > cmpid  && flag == 0) EMPI_Test(&req,&flag,&status);
						} while(flag == 0);
					}
		...
		else if(data->type == MPI_FT_ALLTOALL)
		{
				...
				do
				{
					...
					if(data->id > repid && repid >= 0)
					{
						...
						if(commnC > commnR && commnR > 0)
						{
							...
							{
								completereq[i] = false;
								retValinter = check_errhandler_conditions_collective(((data->args).alltoall.comm->OMPI_CMP_NO_REP_INTERCOMM),&reqs[i]);
								if(retValinter == 0)
								{
						...
						retVal = check_errhandler_conditions_collective(((data->args).alltoall.comm)->OMPI_COMM_REP,&req);
						if(retVal == 0)
						{
			...
			{	
				do
				{
					...
					if(data->id > cmpid) retVal = check_errhandler_conditions_collective(((data->args).alltoall.comm)->OMPI_COMM_CMP,&req);
					if(retVal == 0)
					{
						...
						do
						{
							if(data->id > cmpid && flag == 0) retVal = check_errhandler_conditions_collective(((data->args).alltoall.comm)->OMPI_COMM_CMP,&req);
							if(retVal != 0) break;
							if(data->id > cmpid  && flag == 0) EMPI_Test(&req,&flag,&status);
						} while(flag == 0);
					}
		...
		else if(data->type == MPI_FT_ALLTOALLV)
		{
				...
				do
				{
					...
					if(data->id > repid && repid >= 0)
					{
						...
						if(commnC > commnR && commnR > 0)
						{
							...
							{
								completereq[i] = false;
								retValinter = check_errhandler_conditions_collective(((data->args).alltoallv.comm->OMPI_CMP_NO_REP_INTERCOMM),&reqs[i]);
								if(retValinter == 0)
								{
						...
						retVal = check_errhandler_conditions_collective(((data->args).alltoallv.comm)->OMPI_COMM_REP,&req);
						if(retVal == 0)
						{
			...
			{	
				do
				{
					...
					if(data->id > cmpid) retVal = check_errhandler_conditions_collective(((data->args).alltoallv.comm)->OMPI_COMM_CMP,&req);
					if(retVal == 0)
					{
						if(data->id > cmpid) retVal = EMPI_Ialltoallv ((data->args).alltoallv.sendbuf, (data->args).alltoallv.sendcounts, (data->args).alltoallv.sdispls, (data->args).alltoallv.senddt->edatatype, (data->args).alltoallv.recvbuf, (data->args).alltoallv.recvcounts, (data->args).alltoallv.rdispls, (data->args).alltoallv.recvdt->edatatype, ((data->args).alltoallv.comm)->EMPI_COMM_CMP, &req);
						int flag = !(data->id > cmpid);
						do
						{
							if(data->id > cmpid && flag == 0) retVal = check_errhandler_conditions_collective(((data->args).alltoallv.comm)->OMPI_COMM_CMP,&req);
							if(retVal != 0) break;
							if(data->id > cmpid  && flag == 0) EMPI_Test(&req,&flag,&status);
						} while(flag == 0);
					}
		...
		else if(data->type == MPI_FT_ALLREDUCE)
		{
			if(MPI_COMM_WORLD->EMPI_COMM_CMP != EMPI_COMM_NULL)
			{
				if(data->id > cmpid)
				{
					...
					{
						retVal = 0;
						retVal = check_errhandler_conditions_collective(((data->args).allreduce.comm)->OMPI_COMM_CMP,&req);
				...
				if(data->id > myrepid && myrepid >= 0)
				{
					do
					{
						...
						retVal = check_errhandler_conditions(((data->args).allreduce.comm)->OMPI_CMP_REP_INTERCOMM,&req,cmpToRepMap[rank]);
	...
	*((int *)((curargs->buf) + dis)) = sendid;
	...
	if((comm->EMPI_COMM_REP) != EMPI_COMM_NULL)
	{
		...
		*(curargs->req) = MPI_REQUEST_NULL;
		...
		{
			retVal = 0;
			retVal = check_errhandler_conditions_collective((comm->OMPI_CMP_REP_INTERCOMM),&req);
			if(comm->EMPI_COMM_CMP != EMPI_COMM_NULL)
			{
				if(reqRep != EMPI_REQUEST_NULL) EMPI_Request_free(&reqRep);
...
ConvertToCMPSEND:
		...
		if(!skipcmp || !skiprep)
		{
			do
			{
				...
				if(!skipcmp) retVal = check_errhandler_conditions((comm->OMPI_COMM_CMP),&req,dest);
				...
				if(retVal == 0 && retValRep == 0)
				{
					...
					do
					{
						...
						MPI_Request tmpreq;
						if(last_peertopeer != NULL) tmpreq = *(last_peertopeer->req);
						if(!skipcmp)EMPI_Test(&req,&flag,&status);
						if(cmpToRepMap[dest] != -1 && cmpToRepMap[rank] == -1 && !skiprep) EMPI_Test(&reqRep,&flagRep,&status);
						if(last_peertopeer != NULL) *(last_peertopeer->req) = tmpreq;
					} while(flag == 0);
				}
...
ConvertToCMPSENDSTORED:
		if(nR > 0)
		{
			...
			{
				retValRep = 0;
				retValRep = check_errhandler_conditions_collective((comm->OMPI_CMP_REP_INTERCOMM),&reqRep);
			} while(retValRep != 0);
		}
	}
...
int MPI_Isend (void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *req)
{
	...
	*((int *)((curargs->buf)+dis)) = sendid;
	...
	if((comm->EMPI_COMM_REP) != EMPI_COMM_NULL)
	{
		...
		{
			retVal = 0;
			retVal = check_errhandler_conditions_collective((comm->OMPI_CMP_REP_INTERCOMM),((ftreq)->reqcmp));
			if(comm->EMPI_COMM_CMP != EMPI_COMM_NULL)
			{
...
ConvertToCMPISEND:
		...
		if(!skipcmp || !skiprep)
		{
			do
			{
				...
				retValRep = 0;
				EMPI_Comm_rank(comm->EMPI_COMM_CMP,&rank);
				if(!skipcmp) retVal = check_errhandler_conditions((comm->OMPI_COMM_CMP),((ftreq)->reqcmp),dest);
...
ConvertToCMPISENDSTORED:
		if(nR > 0)
		{
			...
			{
				retValRep = 0;
				retValRep = check_errhandler_conditions_collective((comm->OMPI_CMP_REP_INTERCOMM),((ftreq)->reqrep));
			} while(retValRep != 0);
		}
	}
	*req = ftreq;
...
int MPI_Recv (void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Status *stat)
{
  int retVal,retValRep;
	...
	if((comm->EMPI_COMM_REP) != EMPI_COMM_NULL)
	{
		...
		{
			retValRep = 0;
			if(cmpToRepMap[source] == -1) retValRep = check_errhandler_conditions((comm->OMPI_CMP_REP_INTERCOMM),&reqRep,source);
...
int MPI_Irecv (void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Request *req)
{
	...
	*((ftreq)->reqrep) = EMPI_REQUEST_NULL;
	...
	if((comm->EMPI_COMM_REP) != EMPI_COMM_NULL)
	{
		...
		{
			retValRep = 0;
			if(cmpToRepMap[source] == -1) retValRep = check_errhandler_conditions((comm->OMPI_CMP_REP_INTERCOMM),((ftreq)->reqcmp),source);
	...
	else
	{
		if((ftreq)->comm->EMPI_COMM_REP != EMPI_COMM_NULL)
		{
			if((ftreq)->type == MPI_FT_REQUEST_SEND)
			{
				...
				{
					retValRep = 0;
					retValRep = check_errhandler_conditions(((ftreq)->comm->OMPI_COMM_REP),((ftreq)->reqrep),cmpToRepMap[((struct peertopeer_data*)((ftreq)->storeloc))->target]);
				...
				{
					retVal = 0;
					retVal = check_errhandler_conditions_collective(((ftreq)->comm->OMPI_CMP_REP_INTERCOMM),((ftreq)->reqcmp));
					if((ftreq)->comm->EMPI_COMM_CMP != EMPI_COMM_NULL)
					{
			...
			else if((ftreq)->type == MPI_FT_REQUEST_RECV)
			{
				do
				{
					retVal = 0;
					retValRep = 0;
					if(!(((struct peertopeer_data*)((ftreq)->storeloc))->completecmp)) retVal = check_errhandler_conditions(((ftreq)->comm->OMPI_CMP_REP_INTERCOMM),((ftreq)->reqcmp),((struct peertopeer_data*)((ftreq)->storeloc))->target);
					if((ftreq)->comm->EMPI_COMM_CMP != EMPI_COMM_NULL)
					{
						goto ConvertToCMPTESTRECV;
					}
					if(!(((struct peertopeer_data*)((ftreq)->storeloc))->completerep)) retValRep = check_errhandler_conditions(((ftreq)->comm->OMPI_COMM_REP),((ftreq)->reqrep),cmpToRepMap[((struct peertopeer_data*)((ftreq)->storeloc))->target]);
					if((ftreq)->comm->EMPI_COMM_CMP != EMPI_COMM_NULL)
					{
						goto ConvertToCMPTESTRECV;
					}
				...
				if(tmpflagcmp && tmpflagrep)
				{
					...
					else if(*((ftreq)->reqcmp) != EMPI_REQUEST_NULL)
					{
...
ConvertToCMPTESTSEND:
				do
				{
					retVal = 0;
					retValRep = 0;
					if(!(((struct peertopeer_data*)((ftreq)->storeloc))->completecmp)) retVal = check_errhandler_conditions(((ftreq)->comm->OMPI_COMM_CMP),((ftreq)->reqcmp),((struct peertopeer_data*)((ftreq)->storeloc))->target);
					if(!(((struct peertopeer_data*)((ftreq)->storeloc))->completerep)) retValRep = check_errhandler_conditions(((ftreq)->comm->OMPI_CMP_REP_INTERCOMM),((ftreq)->reqrep),cmpToRepMap[((struct peertopeer_data*)((ftreq)->storeloc))->target]);
				} while(retVal != 0 || retValRep != 0);
...
ConvertToCMPTESTSENDSTORED:
				if(nR > 0)
				{
					...
					{
						retValRep = 0;
						retValRep = check_errhandler_conditions_collective(((ftreq)->comm->OMPI_CMP_REP_INTERCOMM),((ftreq)->reqrep));
					} while(retValRep != 0);
				}
			}
			else if((ftreq)->type == MPI_FT_REQUEST_RECV)
...
ConvertToCMPTESTRECV:
				...
				{
					retVal = 0;
					retVal = check_errhandler_conditions(((ftreq)->comm->OMPI_COMM_CMP),((ftreq)->reqcmp),((struct peertopeer_data*)((ftreq)->storeloc))->target);
				} while(retVal != 0);
				if(!(((struct peertopeer_data*)((ftreq)->storeloc))->completecmp))
				{
					MPI_Request tmpreq = ftreq;
...
int MPI_Reduce (void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, int root, MPI_Comm comm)
{
  ...
  if((comm->EMPI_COMM_REP) != EMPI_COMM_NULL)
  {
    ...
    for(int i = start; i < end; i++)
    {
      ...
      if(rank == targetrep && targetrep != -1)
      {
        if(repToCmpMap[rank] != i && !haveData[i])
        {
          ...
          {
            retVal = 0;
            retVal = check_errhandler_conditions((comm->OMPI_CMP_REP_INTERCOMM),&req,i);
    ...
    { 
      retVal = 0;
      retVal = check_errhandler_conditions_collective(OMPI_COMM_REP,&req);
    ...
    last_collective->ocomm = &OMPI_COMM_REP;
    ...
    do
    {
      ...
      retVal = check_errhandler_conditions((comm->OMPI_CMP_REP_INTERCOMM),&rreq,repToCmpMap[rank]);
...
ConvertToCMPREDUCE:
    ...
    if(targetrep != -1)
    {
      if(cmpToRepMap[rank] != targetrep)
      {
        ...
        {
          retVal = 0;
          retVal = check_errhandler_conditions((comm->OMPI_CMP_REP_INTERCOMM),&req,targetrep);
    ...
    {
      retVal = 0;
      retVal = check_errhandler_conditions_collective((comm->OMPI_COMM_CMP),&req);
...
//ConvertToCMPALLREDUCESYNC:
    ...
    do
    {
      ...
      retVal = check_errhandler_conditions((comm->OMPI_CMP_REP_INTERCOMM),&sreq,cmpToRepMap[rank]);
...
/*int MPI_Allreduce(void *sendbuf, void * recvbuf, int count, MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)
{
  ...
  if((comm->EMPI_COMM_REP) != EMPI_COMM_NULL)
  {
		do
    {
			...
			{
				tmpbuf = malloc(count*datatype->size);
				retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_NO_REP_INTERCOMM),&reqinter);
				if(retValinter == 0)
				{
      ...
      retVal = 0;
      retVal = check_errhandler_conditions_collective((comm->OMPI_COMM_REP),&req);
      if(retVal == 0)
      {
        retVal = EMPI_Ireduce (sendbuf, recvbuf, count, datatype->edatatype, op->eop, 0, (comm->EMPI_COMM_REP), &req);
			...
			if(rank == 0)
			{
				...
				do
				{
					if(flag == 0) retVal = check_errhandler_conditions_collective((comm->OMPI_COMM_REP),&req);
					if(retVal != 0) break;
					if(commnC > commnR && commnR > 0 && flaginter == 0) retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_NO_REP_INTERCOMM),&reqinter);
					if(retValinter != 0) break;
					if(flag == 0) EMPI_Test(&req,&flag,&status);
					if(commnC > commnR && commnR > 0 && flaginter == 0) EMPI_Test(&reqinter,&flaginter,&status);
				} while(flag == 0 || flaginter == 0);
			...
			retValintra = 0;
      retValintra = check_errhandler_conditions_collective((comm->OMPI_COMM_REP),&reqintra);
      if(retVal == 0)
      {
        retValintra = EMPI_Ibcast (recvbuf, count, datatype->edatatype, 0, (comm->EMPI_COMM_REP), &reqintra);
			...
			if(rank != 0)
			{
				...
				do
				{
					...
					if(commnC > commnR && commnR > 0 && flaginter == 0) retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_NO_REP_INTERCOMM),&reqinter);
					if(retValinter != 0) break;
					if(flagintra == 0) retValintra = check_errhandler_conditions_collective((comm->OMPI_COMM_REP),&reqintra);
...
ConvertToCMPALLREDUCE:
		do
    {
      ...
      retVal = check_errhandler_conditions_collective((comm->OMPI_COMM_CMP),&req);
			...
			if(commnR > 0 && cmpToRepMap[rank] == -1) retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_NO_REP_INTERCOMM),&req);
      if(retVal == 0 && retValinter == 0)
      {
        retVal = EMPI_Iallreduce (sendbuf, recvbuf, count, datatype->edatatype, op->eop, (comm->EMPI_COMM_CMP), &req);
		...
		do
    {
      ...
      do
      {
				if(flaginter == 0 && commnR > 0 != -1 && cmpToRepMap[rank] == -1) retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_NO_REP_INTERCOMM),&reqinter);
				if(retValinter != 0) break;
  ...
  if((comm->EMPI_COMM_REP) != EMPI_COMM_NULL)
  {
		do
    {
			...
			if(commnC > commnR && commnR > 0)
			{
				...
				{
					completereq[i] = false;
					retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_NO_REP_INTERCOMM),&reqs[i]);
					if(retValinter == 0)
					{
...
int MPI_Bcast (void *buffer, int count, MPI_Datatype datatype, int root, MPI_Comm comm)
{
  ...
  {		
		do
    {
      retVal = 0;
			EMPI_Comm_rank(comm->EMPI_COMM_REP,&rank);
			if(cmpToRepMap[root] == -1) retVal = check_errhandler_conditions_collective((comm->OMPI_CMP_REP_INTERCOMM),&reqinter);
			else retVal = check_errhandler_conditions_collective((comm->OMPI_COMM_REP),&req);
...
ConvertToCMPBCAST:
    do
    {
      retVal = 0;
			...
			EMPI_Comm_rank((comm->EMPI_COMM_CMP),&rank);
      retVal = check_errhandler_conditions_collective((comm->OMPI_COMM_CMP),&req);
			if(cmpToRepMap[root] == -1 && nR > 0) retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_REP_INTERCOMM),&reqinter);
		...
		do
    {
      ...
      do
      {
				if(cmpToRepMap[root] == -1 && flaginter == 0 && nR > 0) retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_REP_INTERCOMM),&reqinter);
				if(retValinter != 0) break;
		...
		do
    {
      retVal = 0;
			...
			if(cmpToRepMap[root] != -1)
			{
				...
				retVal = check_errhandler_conditions_collective((comm->OMPI_COMM_REP),&req);
				if(commnC > commnR && commnR > 0) retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_NO_REP_INTERCOMM),&req);
				if(retVal == 0 && retValinter == 0)
				{
					...
					do
					{
						...
						if(retVal != 0) break;
						if(flag == 0) EMPI_Test(&req,&flag,&status);
						if(commnC > commnR && commnR > 0 && flaginter == 0) retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_NO_REP_INTERCOMM),&req);
						if(retValinter != 0) break;
						if(commnC > commnR && commnR > 0 && flaginter == 0)EMPI_Test(&reqinter,&flaginter,&status);
					} while(flag == 0 || flaginter == 0);
				}
...
ConvertToCMPGATHER:
		do
    {
      ...
      retVal = check_errhandler_conditions_collective((comm->OMPI_COMM_CMP),&req);
			if(cmpToRepMap[root] != -1 && cmpToRepMap[rank] == -1) retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_NO_REP_INTERCOMM),&req);
      if(retVal == 0 && retValinter == 0)
      {
		...
		do
    {
      ...
      do
      {
				if(flaginter == 0 && cmpToRepMap[root] != -1 && cmpToRepMap[rank] == -1) retValinter = check_errhandler_conditions_collective((comm->OMPI_CMP_NO_REP_INTERCOMM),&reqinter);
				if(retValinter != 0) break;
		...
		do
    {
      ...
      if(retVal == 0)
      {
        retVal = EMPI_Ialltoallv (sendbuf, tmpscounts, tmpsdispls, sendtype->edatatype, recvbuf, tmprcounts, tmprdispls, recvtype->edatatype, (comm->EMPI_COMM_REP), &req);
				...
				{
					memcpy((curargs->args).alltoallv.sendbuf + (sendcounts[i-1]*ssize),sendbuf + (sdispls[i]*ssize),(sendcounts[i]*ssize));
				}
