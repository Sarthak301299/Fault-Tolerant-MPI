	...
	while(true) {
		...
		if (pfd.revents != 0) {
			...
			if(pfd.revents & POLLIN) {
				...
				check(bytes_read,"recv error safe to pass");
				int cmd = *((int *)buffer);
...
int poll(struct pollfd *fds, nfds_t nfds, int timeout) {
	...
	if(!server_addr_set) {
		...
		strcpy(server_addr.sun_path,"#EMPIServer");
		server_addr.sun_path[0] = 0;
		
		check((parep_mpi_coordinator_sock = socket(AF_UNIX, SOCK_STREAM, 0)),"Failed to create socket");
		...
		if(pipe(pollintrpipe) < 0) {
			perror("Pollintrpipe creation failed\n");
			exit(1);
		}
	...
	if((timeout >= 0) || (num_active_fds > 0) || (safe_to_pass)) {
		...
		for(int i = 0; i < nfds; i++) {
			if(real_fds[i].revents != 0) {
				fds[i].revents |= real_fds[i].revents;
				if((real_fds[i].revents & POLLHUP) || (real_fds[i].revents & POLLERR)) {
					bad_sock_detected = 1;
				}
			}
		}
