...
int parep_mpi_infiniband_enabled(void) { return 1; }

void infiniband_plugin_exec(int cmd) {
  switch (cmd) {
  case INFINIBAND_EXIT:
    cleanup();
    break;

  case INFINIBAND_PRECHECKPOINT:
    pre_checkpoint();
    break;

  case INFINIBAND_POSTRESTART:
    post_restart();
    break;
      ...
      for (w = list_begin(&lid_list);
        ...
        if (mapping->port != 0) {
          ret = NEXT_IBV_FNC(ibv_query_port)(internal_ctx->real_ctx,
                                             mapping->port, (struct _compat_ibv_port_attr *)(&port_attr));
          if (ret != 0) {
      ...
      if (port_attr.state == IBV_PORT_ARMED ||
        ...
        IBV_DEBUG("Sending virtual lid: 0x%04x, "
            ...
            mapping->real_lid);
						...
						/*Save locally and dump to ckpt instead!!!*/
            ...
            struct ibv_port_attr *port_attr)
{
  ...
  ret = NEXT_IBV_FNC(ibv_query_port)(internal_ctx->real_ctx,
                                     port_num, (struct _compat_ibv_port_attr *)(port_attr));
  if (ret == 0 &&
